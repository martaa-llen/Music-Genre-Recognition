...
def get_tids_from_directory(audio_dir):
    ...
    tids = [] # list of track IDs to include in the dataset
    for _, dirnames, files in os.walk(audio_dir):
        if dirnames == []:
            tids.extend(int(file[:-4]) for file in files) # remove '.mp3'
    return tids
...
def get_audio_path(audio_dir, track_id):
    ...
    tid_str = '{:06d}'.format(track_id) # '000002'
    return os.path.join(audio_dir, tid_str[:3], tid_str + '.mp3') # '../data/fma_small/000/000002.mp3'

#Spectogram functions

def create_spectogram(track_id, AUDIO_DIR): 
    filename = get_audio_path(AUDIO_DIR, track_id) # get path to audio file
    y, sr = librosa.load(filename) # load audio
    spect = librosa.feature.melspectrogram(y=y, sr=sr,n_fft=2048, hop_length=1024)  # create spectogram
    spect = librosa.power_to_db(spect, ref=np.max) # convert to db
    return spect.T # transpose

def plot_spect(track_id, AUDIO_DIR): 
    spect = create_spectogram(track_id, AUDIO_DIR) # create spectogram
    print(spect.shape) # (time frames, frequency bins)
    plt.figure(figsize=(10, 4)) # set size
    librosa.display.specshow(spect.T, y_axis='mel', fmax=8000, x_axis='time') # plot
